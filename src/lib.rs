...
pub fn pull_data() -> Result<()> {
    for s in [
        BackendSelection::VendorHardwareService,
        BackendSelection::SystemJavaService,
    ] {
        let backend = PowerStats::new_with_backend(s)?;

        let gpu_meters = backend
            .energy_meters()?
            .into_iter()
            .filter(|m| m.subsystem == "GPU")
            .collect::<Vec<_>>();
        let gpu_consumers = backend
            .energy_consumers()?
            .into_iter()
        ...
        let meter_readings = backend.read_energy_meters(&meter_ids)?;
        println!("{s:?} GPU meter reading(s): {:?}", meter_readings);

        let consumer_ids = gpu_consumers.iter().map(|c| c.id).collect::<Vec<_>>();
        let consumer_readings = backend.read_energy_consumers(&consumer_ids)?;
